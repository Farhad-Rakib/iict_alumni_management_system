...
export const NotificationSettingsPage: React.FC = () => {
  const { notifications, updateNotifications, hydrateSection } = useSettingsStore();
  const [form, setForm] = useState(notifications);

  useQuery({
    queryKey: ['site-settings', 'notifications'],
    queryFn: () => siteSettingsApi.getSetting('notifications'),
    onSuccess: (payload) => {
      hydrateSection('notifications', payload.setting_value, payload.updated_at);
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      siteSettingsApi.updateSetting('notifications', {
        setting_value: form,
    ...
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
          <Bell className="w-6 h-6 text-[#006A4E]" /> Notification Settings
        </h1>
